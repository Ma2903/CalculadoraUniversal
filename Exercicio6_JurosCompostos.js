...
export default function Exercicio6_JurosCompostos({ navigation, route }) {
  ...
  const calcularJurosCompostos = () => {
    ...
    if (!isNaN(valorCapital) && !isNaN(valorTaxa) && !isNaN(valorPeriodo) && 
      ...
      const montanteCalculado = valorCapital * Math.pow(1 + taxaDecimal, valorPeriodo);
      const jurosCalculados = montanteCalculado - valorCapital;
      
      setMontante(montanteCalculado.toFixed(2));
      setJuros(jurosCalculados.toFixed(2));
    } else {
      setMontante("Valores inválidos");
      setJuros("");
    }
    setCapital("");
    setTaxa("");
  ...
  return (
    <SafeAreaView style={styles.container} >
      <Card style={styles.card}>
        <Card.Content>
          <Title style={[styles.title, { color: color }]}>Calculadora de Juros Compostos</Title>
          <TextInput 
